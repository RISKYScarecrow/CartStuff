...
public class Wheel extends Item {
	...
	public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World,
		...
		float f2 = par3EntityPlayer.prevRotationYaw
				...
				* f;
		double d0 = par3EntityPlayer.prevPosX
		...
		double d1 = par3EntityPlayer.prevPosY
				...
				* (double) f + 1.62D - (double) par3EntityPlayer.yOffset;
		...
		Vec3 vec31 = vec3.addVector((double) f7 * d3, (double) f6 * d3,
				(double) f8 * d3);
		MovingObjectPosition movingobjectposition = par2World.rayTraceBlocks(
				vec3, vec31, true);
		...
		if (movingobjectposition == null) {
			...
			List list = par2World.getEntitiesWithinAABBExcludingEntity(
					...
					par3EntityPlayer.boundingBox.addCoord(vec32.xCoord * d3,
							vec32.yCoord * d3, vec32.zCoord * d3).expand(
							(double) f9, (double) f9, (double) f9));
			int i;
			...
			for (i = 0; i < list.size(); ++i) {
				...
				if (entity.canBeCollidedWith()) {
					...
					AxisAlignedBB axisalignedbb = entity.boundingBox.expand(
							(double) f10, (double) f10, (double) f10);
			...
			if (flag) {
				...
				if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
					...
					EntityHorseCart entityhorsecart = new EntityHorseCart(par2World,
							(double) ((float) i + 0.5F),
							(double) ((float) j + 1.0F),
							(double) ((float) k + 0.5F));
					
					entityhorsecart.rotationYaw = (float) (((MathHelper
							.floor_double((double) (par3EntityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);
					
					entityhorsecart.motionY = 2.0f;

					if (!par2World.getCollidingBoundingBoxes(entityhorsecart,
							entityhorsecart.boundingBox.expand(-0.1D, -0.1D, -0.1D))
							.isEmpty()) {
						return par1ItemStack;
					}

					if (!par2World.isRemote) {
						par2World.spawnEntityInWorld(entityhorsecart);
					}
